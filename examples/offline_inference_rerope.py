...
def setup_environment_variables():
    ...
    rerope_config.update_rerope_params(
        rerope_window = 32768,
        training_length = 32768
    )

    global data_dir
    data_dir = os.getenv("DATA_DIR", "/home/wx/data/kv_cache")
...
def main():
    ...
    with build_llm_with_uc(module_path, name, model) as llm:
        ...
        data_all = []
        with open("/home/wx/va_clean/data/multifieldqa_zh.jsonl", 'r', encoding='utf-8') as f:
            for line in f:
        ...
        for idx in range(num_batch):
            ...
            materials.append("\n\n".join(
                [f"【语料{i+1}】\n{item.get('context', '')}" for i, item in enumerate(data)]
            ))
            questions.append("\n".join(
                [f"{i+1}. {item.get('input', '')}" for i, item in enumerate(data[:15])]
            ))
            references.append([f"{i+1}. {item.get('answers', '')}" for i, item in enumerate(data[:15])])

        system_prompt = "你是一个AI助手，请根据以下材料回答问题。"
        tokenized_inputs = []
        for material, question in zip(materials, questions):
            content = ("请根据以下文本内容回答后面的问题：\n\n"
                       "【文本内容开始】\n"
                       f"{material}\n"
                       "【文本内容结束】\n\n"
                       "请回答以下问题：\n"
